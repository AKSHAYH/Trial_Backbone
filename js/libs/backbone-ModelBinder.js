// Change 1 - ln 420 - added this to stop checkboxes from firing the change event when the model is binding
            ...
            if(el.attr('type')){
                switch (el.attr('type')) {
                    case 'radio':
                        ...
                        break;
                    case 'checkbox':
                        // Change 1 - added this to stop checkboxes from firing the change event when the model is binding
                         // must defer the change trigger or the change will actually fire with the old value
                        el.attr('loading') || el.prop('checked') === !!convertedValue || _.defer(function() { el.trigger('change') });
